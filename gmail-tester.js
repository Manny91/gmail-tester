...
async function _get_recent_email(
  credentials_json,
  token_path,
  options = {
    include_body: true,
  ...
  if (gmail_emails && gmail_emails.length > 0) {
    for (const gmail_email of gmail_emails) {
      const emailFrom = gmail_email.payload.headers.find(
        h => h.name === "From"
      );
      const emailSubject = gmail_email.payload.headers.find(
        h => h.name === "Subject"
      );
      ...
      if (options.include_body) {
        ...
        if (body.size) {
          switch (gmail_email.payload.mimeType) {
            case "text/html":
              email_body.html = Buffer.from(body.data, "base64").toString(
                "utf8"
              );
              break;
            case "text/plain":
            default:
              email_body.text = Buffer.from(body.data, "base64").toString(
                "utf8"
              );
              break;
          }
